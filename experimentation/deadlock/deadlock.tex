...
In this experiment a MultiChain node tries to senst a request to a different node to create a block.
The other node is specifically configured for the purpose of the experiment
to ignore and drop all requests.
The first node now should wait for the response of the other node for a specific time.
During the time that the first node is waiting a different request is sent from a normal node 3 to node 1.
This request will be dropped by the first node,
as it cannot process while node 1 has an outstanding request.
