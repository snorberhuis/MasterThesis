...
\frametitle{Atomic operations on the chain}
    \begin{itemize}
        ...
        \pauze \item Only atomic operations are allowed on the chain. This prevents branches.
        \pauze \item A peer drops any incoming requests during outstanding request. This is a Drop Event.
    \end{itemize}
\end{frame}
\note{
...
But MultiChain will wait for a certain amount of data before it would schedule a block.

MultiChain is designed to only schedule to create blocks above a certain threshold.
...
This number of hops can be varied to increase anonymity at the cost of download speed.
...
\end{frame}
\note{
Here the result of the experiment is plotted.
Again the download of every block is plotted on the left, Now the upload amounts are also plotted on the right.
MultiChain correctly tracks the download until 92% of the download.
...
But if some one has both blocks containing the double hash pointer,
...
For example this could be an example of a the chain of the malicious node.
...
\begin{frame}
    ...
    \begin{itemize}
        \pause \item Add queuing to minimize drop events.
...
\end{frame}
\note{
So now I will shortly summarize some recommendations I have for the next steps to make MultiChain production ready.

First, a queue should be added that prevents messages to be dropped.
